...
public class AudioManager : MonoBehaviour
{
    ...
    {
        if (audioMixer == null)
            Log.LogError("[AudioManager] AudioMixer가 할당되지 않았습니다. Inspector에서 연결해 주세요.");
    ...
    public void PlayBGMWithFade(string key, float fadeDuration = 1.0f, bool loop = true)
    {
        AudioClip clip = bgmLibrary.Get(key)?.clip;
    ...
    private AudioSource GetAvailablePoolSource()
    {
        ...
        Log.LogWarning("[AudioManager] SFX 풀 초과 — 새 AudioSource를 생성합니다.");
